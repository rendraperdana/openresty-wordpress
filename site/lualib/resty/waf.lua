...
local function _process_rule(self, rule, collections, ctx)
	...
	for k, v in ipairs(vars) do
		...
		if not collection then
			...
			if match then
				if self._debug == true then ngx.log(self._debug_log_level, '[', self.transaction_id, '] ', "Match of rule " .. id) end
	...
	for _, ruleset in ipairs(self._active_rulesets) do
		...
		while rule do
			...
			if not util.table_has_key(id, self._ignore_rule) then
				if self._debug == true then ngx.log(self._debug_log_level, '[', self.transaction_id, '] ', "Processing rule " .. id) end
				...
				if self._debug == true then ngx.log(self._debug_log_level, '[', self.transaction_id, '] ', "Ignoring rule " .. id) end
